...
def train_model():
    ...
    clf_lgb = lgb.LGBMClassifier(random_state=4262, objective="binary", scale_pos_weight=1)

    # Recursively eliminate features with cross validation
    rfecv = RFECV(estimator=clf_lgb, cv=5, scoring='average_precision', n_jobs=-1, verbose=10, step=1, min_features_to_select= min_features_to_select)
    ...
    clf_lgb = lgb.LGBMClassifier(objective="binary", random_state=4262, boosting_type="gbdt", n_estimators=1000)
    ...
    gscv_model = GridSearchCV(clf_lgb, param_grid = best_params, verbose =10, cv=5, scoring = 'average_precision', n_jobs=-1)
    ...
    clf_lgb = gscv_model.best_estimator_
    clf_lgb.fit(X_train, y_train, eval_set=[(X_val, y_val)], eval_metric = "aucpr", verbose=False, early_stopping_rounds=50)
    ...
    print("Performance of LGBM on test set:")
    ...
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    path = os.path.join(outdir, "trained_model_lgb.bin")
    
    print(f"Saving model to {path}...")
    clf_lgb.booster_.save_model(path)
    ...
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    path = os.path.join(outdir, "RFEfeatures_lgb.json")
